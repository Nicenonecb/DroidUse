...
文件选择、应用/链接打开、文本编辑与 IME 操作已定义任务层接口和能力校验；并不表示当前后端能实际执行所有动作。原厂系统仍需 ADB 激活手机端实验执行器；独立中文输入、完整系统资源隔离以及生产级跨应用执行仍待 ROM 集成。尚未编译或刷入自建 ROM。进程退出后的检查点用于说明中断状态，不会自动重放或接续旧任务。
