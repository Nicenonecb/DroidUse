...
当前文字输入使用电脑控制会话的系统输入通道，触摸、按键、文字、删除和回车已经通过 Soong 编译；2026-09-23 已通过中文插入、截图、点击、滑动、暂停恢复和异常清理的实机验证，见 [M2 联调记录](../../docs/m2-validation-2026-09-23.md)；复杂编辑行为仍需验证。保护心跳和分阶段故障回收已实现。UI 语义树、剪贴板、实体屏幕输入和 IME 编辑代次的完整验收仍未完成。
...
该工具不会修改 AOSP、不会复制私钥，也不会生成表示 SELinux 已通过的活动标记。服务器锁定分支已完成 Soong 编译；M2 基础真机验证已经完成，但全面安全与资源隔离验收尚未完成，因此当前产品配置仍不安装 `sepolicy-version` 活动标记。
